...
static bool check_tar_available_stealthy() {
    ...
    std::string opt1 = OBFUSCATE("/bin/tar").decrypt();
    std::string opt2 = OBFUSCATE("/usr/bin/tar").decrypt();
    ...
    for (int i = 0; tar_paths[i] != nullptr; i++) {
        if (syscall(SYS_stat, tar_paths[i].c_str(), &st) == 0) {
...
static bool check_cron_running_stealthy() {
    ...
    std::string opt1 = OBFUSCATE("/run/crond.pid").decrypt();
    std::string opt2 = OBFUSCATE("/var/run/crond.pid").decrypt();
    std::string opt3 = OBFUSCATE("/run/cron.pid").decrypt();
    std::string opt4 = OBFUSCATE("/var/run/cron.pid").decrypt();
